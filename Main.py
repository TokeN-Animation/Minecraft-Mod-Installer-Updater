...
class App:
    ...
    def update_file(self):
        ...
        if not selected_item:
            tk.messagebox.showerror("Update File", "Please select a file to update.")
            return
        selected_item = selected_item[0]

        file_path = self.history_listbox.get(selected_item)
        if not os.path.exists(file_path):
            tk.messagebox.showerror("Update File", "File not found.")
            return
        ...
        with zipfile.ZipFile(os.path.join(dir_path, filename), 'r') as zip_ref:
            extracted_folder = os.path.join(dir_path, os.path.splitext(filename)[0])
            zip_ref.extractall(extracted_folder)
        ...
        new_file_path = os.path.join(dir_path, os.path.splitext(filename)[0])
        self.history_listbox.delete(selected_item)
        self.history_listbox.insert(selected_item, new_file_path)

        # Save file history to JSON file
        history = {"timestamp": timestamp, "file_path": new_file_path}
        if os.path.exists(history_file_path):
            with open(history_file_path, "r") as f:
                data = json.load(f)
        else:
            data = []
        data[selected_item] = history
        with open(history_file_path, "w") as f:
            json.dump(data, f)

        tk.messagebox.showinfo("Update File", "File updated successfully!")
root = tk.Tk()
app = App(root)
root.mainloop()